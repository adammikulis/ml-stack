...
1. ``$MAINSPRING_CONTRACTS`` -- an explicit directory. Set this in a deployment that ships
...
def _candidates() -> list[Path]:
    ...
    override = os.environ.get("MAINSPRING_CONTRACTS")
    if override:
...
def contracts_dir() -> Path:
    ...
    raise ContractError(
        ...
        + "). Set MAINSPRING_CONTRACTS to point at one."
    )
...
def reset_cache() -> None:
    """Drop the parse cache. For tests that point MAINSPRING_CONTRACTS somewhere else."""
    _CACHE.clear()
