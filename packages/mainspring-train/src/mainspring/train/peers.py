"""``mainspring-peers`` -- set up a cluster and see who is in it.

    mainspring-peers init     # mint the key, print the command to run elsewhere
    mainspring-peers ls       # who is on this LAN right now
    mainspring-peers token    # the bearer token this key derives
...
def _require_key(path: str | None) -> bytes:
    ...
    if key is None:
        raise DiscoveryError(
            f"no cluster key at {key_path(path)} -- run 'mainspring-peers init'")
    return key
...
def cmd_ls(args: argparse.Namespace) -> int:
    ...
    if not peers:
        ...
        print("  - is 'mainspring-traind' running there?")
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(prog="mainspring-peers")
