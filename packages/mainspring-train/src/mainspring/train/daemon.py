...
class JobRunner:
    """Serialises jobs onto one device and owns their processes."""
    ...
    def stop(self, job_id: str, *, grace_s: float = 30.0) -> Job:
        with self._lock:
            ...
            if job.state == "queued":
                ...
                job.state = "stopped"
                return job
            proc = self._current if job.state == "running" else None
        if proc is None:
            return job
    ...
    def _run_one(self, job: Job) -> None:
        ...
        env = {**os.environ, **job.env, "PYTHONUNBUFFERED": "1",
               "MAINSPRING_JOB_ID": job.id,
               "MAINSPRING_JOB_DIR": str(self.job_dir(job.id))}
        try:
...
def device_report() -> dict[str, Any]:
    """What card is here, and how much of it is free. Best effort."""
    out: dict[str, Any] = {"backends": []}
    try:
        from mainspring.backend import available
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:                      # noqa: N802
            ...
            if not self._guard():
                return
            if path == "/jobs":
                self._send(200, {"jobs": [j.public() for j in runner.jobs.values()]})
                return
        ...
        def do_PUT(self) -> None:                      # noqa: N802
            ...
            with partial.open(mode) as fh:
                ...
                while remaining > 0:
                    ...
                    fh.write(chunk)
                    remaining -= len(chunk)
            if self.headers.get("X-Mainspring-Complete", "1") == "1":
...
def main(argv: list[str] | None = None) -> int:
    import argparse
    ap = argparse.ArgumentParser(prog="mainspring-traind")
