...
def a_store(tmp_path: pathlib.Path, *, slug: str = "velthorne-open-texts",
           title: str = "Velthorne Open Texts", sections: int = 4, run: str = RUN,
           rows=None, store=None, write_run_node: bool = True, decisions=None):
    """A folded source on a sources, with a run node and (optionally) judged decisions."""
    rows = rows if rows is not None else [
        a_run_read(f"{slug}:1:1.1", source=slug, run=run, prompt=800, completion=200,
                  seconds=40.0,
                  extracted=said("vault", "charge",
                                 relations=[("vault", "produces", "charge")])),
        a_run_read(f"{slug}:1:1.2", source=slug, section="1.2", pages=(4, 5), run=run,
                  prompt=700, completion=150, seconds=46.0,
                  extracted=said("vault", "seam wall",
                                 relations=[("seam wall", "part_of", "vault")])),
    ]
    where = a_part_read_source(tmp_path, slug=slug, title=title, sections=sections, rows=rows,
                             store=store)
...
def test_a_source_is_tabled_with_its_counts_cost_and_run(tmp_path):
    store = a_store(tmp_path)
    source = ingest.Sources(store).source("velthorne-open-texts")
    assert source is not None                                # the fixture folded it

    body = report([], ingested=[str(store)])
    assert "## Ingest" in body
    head = ("| source | read | failed | given up | nodes | edges | seconds | s/unit "
            "| prompt tok | completion tok | tok/unit | run(s) |")
    assert head in body
    assert (f"| Velthorne Open Texts | 2/4 | 0 | 0 | {source.folded_nodes} "
            f"| {source.folded_edges} | 86 | 43.0 | 1500 | 350 | 925 "
...
def test_failed_and_given_up_units_are_counted_apart(tmp_path):
    ...
    progress.source(slug)["done"][f"{slug}:1:1.2"]["attempts"] = ingest.GIVE_UP
    progress.save()
...
def test_a_source_with_no_run_recorded_shows_a_dash(tmp_path):
    slug = "velthorne-open-texts"
    rows = [a_read(f"{slug}:1:1.1", source=slug, extracted=said("vault"))]  # no "run" key
    store = a_store(tmp_path, rows=rows, write_run_node=False)
...
def test_a_run_id_the_store_holds_no_node_for_prints_the_bare_id(tmp_path):
    store = a_store(tmp_path, write_run_node=False)
    body = report([], ingested=[str(store)])
    assert f"| {RUN} |" in body


def test_two_runs_reading_the_same_source_are_both_named(tmp_path):
    slug = "velthorne-open-texts"
    rows = [
        a_run_read(f"{slug}:1:1.1", source=slug, run=RUN, extracted=said("vault")),
        a_run_read(f"{slug}:1:1.2", source=slug, section="1.2", pages=(4, 5), run=RUN2,
...
def test_the_total_line_sums_across_its_sources(tmp_path):
    store = tmp_path / "sources.ladybug"
    a_store(tmp_path, store=store)
    a_store(tmp_path, store=store, slug="lattice-studies", title="Lattice Studies",
           sections=2, run=RUN,
           rows=[a_run_read("lattice-studies:1:1.1", source="lattice-studies", run=RUN,
                            prompt=300, completion=100, seconds=20.0,
                            extracted=said("glimmer node"))])

    body = report([], ingested=[str(store)])
    assert ("2 source(s), 3/6 unit(s) read, 0 failed (0 given up), 106s (35.3 s/unit), "
...
def test_the_judged_decisions_count_is_read_when_the_hygiene_pass_has_run(tmp_path):
    store = a_store(tmp_path, decisions={
...
def test_no_sources_argument_prints_no_section():
    body = report([])
    assert "## Ingest" not in body


def test_a_store_naming_nothing_prints_no_section(tmp_path):
    """A ``--sources`` pointed at a store with no source behind it -- nothing ever ingested --
    is left out entirely, the way an empty ``extracted`` leaves out "Extraction": a
    heading over nothing reads as a source that was read and found empty."""
    body = report([], ingested=[str(tmp_path / "nothing-here.ladybug")])
    assert "## Ingest" not in body


def test_one_empty_store_beside_one_real_one_only_tables_the_real_one(tmp_path):
    store = a_store(tmp_path)
    body = report([], ingested=[str(tmp_path / "nothing-here.ladybug"), str(store)])
...
def test_ingest_sits_beside_answering_runs_in_the_same_document(tmp_path):
    ...
    ingested_store = a_store(tmp_path, store=tmp_path / "sources.ladybug")
    body = report(kept, ingested=[str(ingested_store)])
...
def test_the_text_rendering_carries_the_same_numbers_without_pipes(tmp_path):
    store = a_store(tmp_path)
    body = report([], ingested=[str(store)], md=False)
    assert "|" not in body
    assert "INGEST" in body
    assert "Velthorne Open Texts" in body
