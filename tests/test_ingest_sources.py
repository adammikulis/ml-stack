"""Several sources read into one store: what they hold together, what it cost, how often
it is folded, and what happens when the server resets inside one read.

Every source, concept and relation here is invented. Nothing opens a port a model could be
...
def a_second_source(store, *, slug=FIELD_GUIDE, title="Ambleford Field Guide"):
    """A second source beside the first, naming one concept the first also names."""
    rows = [a_read(f"{slug}:1:1.1", source=slug, title="Spore Blooms",
                   extracted=said("vault", "spore bloom",
                                  relations=[("spore bloom", "part_of", "vault")])),
            a_read(f"{slug}:2:2.1", source=slug, chapter="2", section="2.1", pages=(6, 7),
                   title="Blooming", extracted=said("spore bloom", "damp season",
                                                    relations=[("spore bloom", "requires",
                                                                "damp season")]))]
    return a_part_read_source(store.parent, slug=slug, rows=rows, title=title, sections=6,
                            store=store)


def two_sources(tmp_path):
    """Two sources folded into one store, sharing the concept both of them name."""
    store = a_part_read_source(tmp_path)
    a_second_source(store)
    assert ingest.fold(store, say=lambda _: None) == 0
    return store


def two_sources_spelled_apart(tmp_path):
    """Two sources folded into one store, one spelling `seam wal` what the other spells
    `seam wall`: a letter apart, so the fold leaves them two nodes for the hygiene pass."""
    store = a_part_read_source(tmp_path)
    rows = [a_read(f"{FIELD_GUIDE}:1:1.1", source=FIELD_GUIDE, title="Spore Blooms",
                   extracted=said("spore bloom", "seam wal",
                                  relations=[("spore bloom", "part_of", "seam wal")]))]
    a_part_read_source(tmp_path, slug=FIELD_GUIDE, rows=rows, title="Ambleford Field Guide",
...
def test_the_sources_view_names_each_one_and_what_the_store_holds_for_it(tmp_path):
    store = two_sources(tmp_path)

    got = ingest.Sources(store).shared()

    sources = {b["source"]: b for b in got["sources"]}
    assert set(sources) == {OPEN_TEXTS, FIELD_GUIDE}
    assert sources[OPEN_TEXTS]["title"] == "Velthorne Open Texts"
    assert sources[OPEN_TEXTS]["read"] == 2 and sources[OPEN_TEXTS]["wanted"] == 4
    # vault, charge, seam wall -- and the second source's own three
    assert sources[OPEN_TEXTS]["nodes"] == 3 and sources[FIELD_GUIDE]["nodes"] == 3
    assert sources[OPEN_TEXTS]["edges"] == 2 and sources[FIELD_GUIDE]["edges"] == 2


def test_a_concept_two_sources_name_is_listed_with_the_sources_that_name_it(tmp_path):
    store = two_sources(tmp_path)
...
def test_the_pairs_the_hygiene_pass_judged_are_counted(tmp_path):
    ...
    judged = ingest.Sources(store).shared()["decisions"]
...
def test_a_store_no_pass_has_judged_counts_no_pairs(tmp_path):
    ...
    assert ingest.Sources(store).shared()["decisions"]["pairs"] == 0


# -- a name joined across sources on the way in ----------------------------------------------


def two_sources_folded_in_turn(tmp_path):
    """One source folded, then a second whose unit names the first source's `seam wall` in the
...
def test_the_fold_writes_each_name_it_lands_on_an_existing_node_with_the_units_of_both(
    ...
    assert one["edges_moved"] == 1, "the relation; the edge to its source is written after"
...
def test_a_name_the_fold_landed_across_two_sources_is_between_them_and_the_command_prints_it(
        tmp_path, capsys):
    store = two_sources_folded_in_turn(tmp_path)
    ingest.main(["fold", "--out", str(store)])

    assert ingest.Sources(store).between_sources() == [
        {"a_label": "seam wall", "a_source": OPEN_TEXTS, "b_label": "seam walls",
         "b_source": FIELD_GUIDE, "kind": "structure", "weight": 5}], \
        "the joined node's four mentions, two from each source, and the one edge rewired"
    assert ingest.main(["sources", "--out", str(store)]) == 0
    said_out = capsys.readouterr().out
    assert "between sources (1)" in said_out
...
def test_a_source_folded_again_keeps_what_the_other_source_gave_a_shared_node(tmp_path):
    ...
    assert ingest.Sources(store).between_sources()[0]["weight"] == 5
...
def test_a_source_read_further_grows_its_own_share_of_a_shared_node(tmp_path):
    ...
    store = two_sources_folded_in_turn(tmp_path)
    ingest.main(["fold", "--out", str(store)])
    rows = ingest.Sources(store).reads(OPEN_TEXTS)
...
def test_a_fold_in_memory_a_dry_fold_and_a_read_only_absorb_write_no_landing(tmp_path):
    ...
    graph = sources.graph(FIELD_GUIDE)
...
# -- a name joined across sources by the hygiene pass ------------------------------------------
...
def test_tidy_writes_each_merge_it_makes_to_the_store_with_the_units_both_names_came_from(
    ...
    assert one["kind"] == "structure"
    assert one["edges_moved"] == 2, "the relation, and the read_from edge to its source"
    assert one["kept_from"] == [f"{OPEN_TEXTS}:1:1.2"]
    assert one["gone_from"] == [f"{FIELD_GUIDE}:1:1.1"]
    assert one["at"]
...
def test_a_name_merged_across_two_sources_is_between_them_with_a_weight(tmp_path):
    ...
    assert got["merged"] == ingest.Sources(store).between_sources()
    assert got["merged"] == [{"a_label": "seam wall", "a_source": OPEN_TEXTS,
                              "b_label": "seam wal", "b_source": FIELD_GUIDE,
                              "kind": "structure", "weight": 6}], \
        "the joined node's four mentions, two from each source, and the two edges moved"
...
def test_a_store_never_tidied_has_nothing_between_sources(tmp_path):
    assert ingest.Sources(two_sources_spelled_apart(tmp_path)).between_sources() == []


def two_sources_a_plural_apart(tmp_path):
    """Two sources folded into one store, one naming `seam walls` what the other names
...
def test_the_sources_command_prints_the_sources_the_shared_concepts_and_the_judged_pairs(
        tmp_path, capsys):
    store = two_sources(tmp_path)
...
def test_the_sources_command_asks_for_a_tidy_once_a_merge_is_logged_within_one_source(
    ...
    with GraphStore(store, read_only=True) as held:
        assert len(held.get_doc(MERGES)["merges"]) == 1, \
            "a merge is logged, none across sources"
...
def test_the_sources_command_takes_a_sample_size(tmp_path, capsys):
    store = two_sources(tmp_path)
    a_part_read_source(tmp_path, slug="kessleton-mills", title="Kessleton Mills", store=store,
                       rows=[a_read("kessleton-mills:1:1.1", source="kessleton-mills",
                                  extracted=said("vault", "charge"))])
    ingest.fold(store, say=lambda _: None)

    assert ingest.main(["sources", "--out", str(store), "--sample", "1"]) == 0

    said_out = capsys.readouterr().out
    assert "concepts in more than one source (2)" in said_out
    assert "... and 1 more" in said_out


def test_the_sources_command_on_a_store_that_is_not_there_says_so(tmp_path, capsys):
    assert ingest.main(["sources", "--out", str(tmp_path / "nothing.ladybug")]) == 1
    assert "no store at" in capsys.readouterr().out


def test_the_sources_command_needs_a_store(capsys):
    assert ingest.main(["sources"]) == 2
    assert "sources needs --out STORE" in capsys.readouterr().err
...
def test_one_sources_tokens_are_added_up_over_the_calls_each_unit_made(tmp_path):
    ...
    source = ingest.Sources(store).source(OPEN_TEXTS)

    assert (source.prompt_tokens, source.completion_tokens) == (2600, 840)
    assert source.tokens == 3440 and source.tokens_per_unit == 1720
    assert source.per_unit == 86.0
...
def test_a_read_from_before_the_calls_were_kept_costs_nothing_rather_than_failing(tmp_path):
    ...
    assert (source.prompt_tokens, source.completion_tokens, source.tokens_per_unit) == (0, 0, 0)


def test_status_says_what_each_source_cost_and_what_the_run_cost(tmp_path, capsys):
    rows = [a_read(f"{OPEN_TEXTS}:1:1.1", extracted=said("vault")),
            a_read(f"{OPEN_TEXTS}:1:1.2", section="1.2", extracted=said("charge"))]
    for row in rows:
    ...
    assert "total: 2,000 read + 500 written token(s) over 2 unit(s)" in said_out


def test_status_totals_the_cost_over_every_source(tmp_path, capsys):
    store = a_part_read_source(tmp_path)
    ...
    said_out = capsys.readouterr().out
    assert "total: 1,000 read + 200 written token(s) over 4 unit(s)" in said_out


# -- how often a source is folded ------------------------------------------------------------
...
def test_the_interval_widens_with_what_the_last_fold_took():
    """A fold of a minute is not paid at every chapter end of a long source."""
...
def test_the_progress_file_records_what_the_fold_took(tmp_path):
    ...
    held = json.loads(ingest.Progress.beside(store).read_text())["sources"][OPEN_TEXTS]
...
def test_a_dry_run_says_what_the_fold_would_cost_and_writes_nothing(tmp_path):
    ...
    assert "folded_seconds" not in json.loads(
        ingest.Progress.beside(store).read_text())["sources"][OPEN_TEXTS]


def a_wide_source(tmp_path, units, *, slug=OPEN_TEXTS, store=None):
    """``units`` units of a source whose vocabulary is as wide as the source is long."""
    ...
    for n in range(units):
        ...
        rows.append(a_read(f"{slug}:{chapter}:{chapter}.{n % 15 + 1}", source=slug,
                           chapter=chapter, section=f"{chapter}.{n % 15 + 1}",
                           pages=(n * 2, n * 2 + 1),
                           extracted=said(*picks, relations=[(picks[i], "part_of", picks[i + 1])
                                                             for i in range(4)])))
    return a_part_read_source(tmp_path, slug=slug, rows=rows, sections=units * 2, store=store)


@pytest.mark.slow
def test_what_a_fold_costs_as_a_source_grows_is_what_the_interval_is_chosen_from(tmp_path,
                                                                              capsys):
    ...
    for units in (300, 1000, 3000):
        ...
        a_wide_source(tmp_path, units, store=store)
        rows = ingest.Sources(store).reads(OPEN_TEXTS)
        began = time.time()
        graph = ingest.fold_source(rows, ingest.units_of(rows))
...
def a_reset(unit):
    return ingest.Read(unit=unit.id, source=unit.source, chapter=unit.chapter,
...
def test_a_run_whose_server_reset_reads_the_unit_again_and_carries_on(tmp_path, server,
                                                                     monkeypatch, capsys):
    """A reset on the first unit costs one extra read; the source is still finished."""
    from test_ingest import a_reading

    source, instance, _asked = a_reading(tmp_path, server)
    ...
    assert in_store(store) >= {"source:lattice", "concept:glimmer-node"}
