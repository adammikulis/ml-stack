...
def test_the_fingerprint_covers_everything_that_shapes_an_answer():
    ...
    on = fingerprint("who surveys?", **base, asking={"constrain_ids": True})
    off = fingerprint("who surveys?", **base, asking={"constrain_ids": False})
    assert on != off and on != key
    assert fingerprint("who surveys?", **base, asking={"tight": True, "constrain_ids": True}) \
        == fingerprint("who surveys?", **base, asking={"constrain_ids": True, "tight": True})
    assert fingerprint("who surveys?", **base, asking={}) == key, "nothing said is the same key"
