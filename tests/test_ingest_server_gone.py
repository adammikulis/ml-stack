"""When the model server goes away mid-sources, the run stops -- it does not write two hundred
...
def test_an_unreachable_server_is_not_the_units_attempt(tmp_path):
    ...
    progress.note("velthorne", gone)
    progress.note("velthorne", gone)
    entry = progress.state["sources"]["velthorne"]["done"][gone.unit]
...
def test_the_run_stops_when_its_server_is_gone(tmp_path, server, monkeypatch, capsys):
    ...
    code = ingest.main([source, "--out", str(store), "--base-url", instance.base_url])
    ...
    entries = next(iter(progress.state["sources"].values()))["done"]
...
def test_a_second_detach_is_refused_while_the_recorded_run_is_still_ending(tmp_path, monkeypatch, capsys):
    ...
    try:
        ...
        assert ingest.main(["source.pdf", "--out", str(tmp_path / "s"), "--detach"]) == 2
