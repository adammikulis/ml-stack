...
| `ml-stack-suite run NAME --seed N` | a measurement run over several seeds and written down whole: the seed, the commit and whether the tree was edited, the wall clock, the peak memory, how busy the card was when it started, and each metric's mean, spread and how many seeds actually produced it. A suite is a function that measures one seed and returns numbers (`@ml_stack.suite.register`); `--import MODULE` brings a project's own in, `--set k=v` reaches its arguments, `--out DIR` writes the result after **every** seed, so a run killed in its third keeps the two it paid for, and two runs of one suite take turns on a lock rather than timing each other |
