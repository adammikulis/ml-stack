"""Printing what was measured: the columns one run is read as, the table, the grouping by
serving, the two-run comparison and the `drafts` summary.

Everything here reads kept runs and writes to stdout; the numbers it prints are `score`'s.
The columns are the lessons -- `ctx`, `n`, `conc` and `find` are each on every line because
a comparison across any of them is two measurements read as one. The questions behind a
score are in `ml_stack.bench.detail`, and the frontier and the plot in
`ml_stack.bench.frontier`.
"""
