...
HELP: dict[str, str] = {
    ...
    "claude": "Claude Code on a model this machine serves, in the settings it scored best with.",
    "do": "A task in words, done by a served model with the ml-stack commands as tools.",
    "draft": "What a draft head is worth for one model, arm by arm, against no head at all.",
...
TABLE: tuple[Row, ...] = (
    ...
    Row("ml-stack-serve limits [--memory 90G] [--servers N] [--slots N] [--idle 10m]",
        "how much of this machine ml-stack may take, written down once and read everywhere: the memory cap rides on `hub.room`, so every preflight, fit and lease honours it without being handed it; the server and slot caps refuse a lease **before** a process is started, naming what to stop or what to raise; `--idle` is what `reclaim` and the fleet daemon act on. Every limit is off until one is set, so a machine nobody has told anything about behaves exactly as it did; `--clear` takes them all off"),
    ...
    Row("ml-stack-graph serve --site FILE",
        "a rendered graph page (`graph.page.render`) served on loopback as a whole document: `GET /` is the page with `window.GRAPH_LIVE` set so it asks this server rather than nothing, `/export/<path>` a file under `--export DIR` (JSON and HTML by their suffix, anything else as bytes, nothing outside the root), and the ask routes -- `/ask`, `/ask/stream`, `/thread/<name>`, `/ask/model`, `/metrics` -- answered over `--graph FILE` with `--model` leased on the first question (`--model-port` is where it is served), every question and answer kept in `PATH.conversations` beside the corpus `--store PATH` names, never in it (`--conversations-in-store` keeps them in it); `--port` picks the listener. `POST /request` appends the page's change request to `--requests FILE` (`PATH.requests.jsonl` beside the store, or beside `--site` without one) and has the model propose edits for it into the review queue `GET /review` lists and `POST /review` accepts, refuses or undoes (`--review FILE`, `PATH.review.json` by default), an accepted edit landing in `--store`; `/refresh` and `/draft` answer once a subclass gives them stages and a drafter"),
    Row("ml-stack-audit",
